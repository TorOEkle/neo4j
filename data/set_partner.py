...
class Person:
    ...
    def set_partner(self, partner, is_married=False):
        ...
        if is_married and self.age < 55 and partner.age < 55:
            combined_name = f"{self.family_name}-{partner.family_name}" if self.sex == "Female" else f"{partner.family_name}-{self.family_name}"
            if not any(name in self.family_name for name in [self.family_name, partner.family_name]):
            ...
            # For adults, combine names with female's name first
                ...
                combined_name = f"{self.family_name}-{partner.family_name}" if self.sex == "Female" else f"{partner.family_name}-{self.family_name}"
                self.family_name = partner.family_name = combined_name


    def set_family_name(self, new_name):
        self.family_name = new_name
        # Update the family name for the children
        for child in self.children:
            if child.family_name == new_name:
                continue
            else:
                child.set_family_name(new_name)

    def add_child(self, child):
        # Add child to this parent's list if not already present
        if child not in self.children:
            self.children.append(child)

        # If this person has a partner, add the child to the partner's list, if not already present
        if self.partner and child not in self.partner.children:
            self.partner.children.append(child)

        # Update the child's parents list
        if self not in child.parents:
            child.parents.append(self)
        if self.partner and self.partner not in child.parents:
    ...
    def __str__(self):
        ...
        parent_names = ', '.join([p.first_name for p in self.parents])
        children_names = ', '.join([c.first_name for c in self.children])
    ...
    for family in families.values():
        ...
        if oldest_male:
            ...
            for member in family.families:
                if member.age < 55:  # Update names for non-grandparents
...
def assign_parents(parents, child):
    ...
    if possible_parents:
        ...
        # Set selected parents and update family names if needed
        selected_pair[0].add_child(child)  # This will also add the child to the partner's list
...
def set_partners(persons):
    ...
    if len(remaining_individuals) ==1:
        remaining_individuals[0].partner
